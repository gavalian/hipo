...
void convert(const std::string &pathInput, const std::string &pathOutput, int compression, bool iofeatures) {
   ...
   while(reader.Next()) {
     ...
     */
      ...
      for (int i = 0; i < count; ++i) {
         double value = sqrt(px->at(i)*px->at(i) + py->at(i)*py->at(i) + pz->at(i)*pz->at(i))*
         sqrt(vx->at(i)*vx->at(i)+vy->at(i)*vy->at(i)+vz->at(i)*vz->at(i))
		+ vt->at(i) + pid->at(i)*beta->at(i)*chi2pid->at(i)+status->at(i) - charge->at(i);
      h->Fill(value);
         /*outPid[i]      = pid->at(i);
         outPx[i]       = px->at(i);
         outPy[i]       = py->at(i);
   ...
   std::cout << "Runtime (process): " << runtime << " us" << std::endl;
}
